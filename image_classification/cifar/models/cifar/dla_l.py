...
class light_layer_attention_module(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.layer_attention = light_layer_attention(input_dim=input_dim, dim_perhead=self.dim_perhead)
        self.lambda_t = nn.Parameter(torch.randn(input_dim, 1, 1))  
        
    def forward(self, xt, ot_1):
        attn_t = self.layer_attention(xt)
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes=1000, block_name='BasicBlock', drop_path=0.0):
        ...
        light_layer_attention = layer_attention
        self.inplanes = 16
        ...
        stages = [None]*3
        stages[0] = Attention(self._make_layer(block, 16, n, stride=1, drop_path=drop_path, light_layer_attention=light_layer_attention), 1)
        stages[1] = Attention(self._make_layer(block, 32, n, stride=2, drop_path=drop_path, light_layer_attention=light_layer_attention), 2)
        stages[2] = Attention(self._make_layer(block, 64, n, stride=2, drop_path=drop_path, light_layer_attention=light_layer_attention), 3)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, drop_path=0.0, light_layer_attention=None):
        downsample = None
        ...
        for i in range(0, blocks):
            attention.append(light_layer_attention(planes, drop_path=drop_path))
        
        return nn.Sequential(*layers), nn.Sequential(*attention)
